...
class EEGNet(torch.nn.Module):
    def __init__(self, n_output,sample_rate,F1,D,F2,n_channels,bias,dropout,activation,norm,batch_size):
        ...
        if norm == "BatchNorm":
            ...
            self.norm1 = nn.LayerNorm([batch_size,F1,n_channels,1025], eps = 1e-05, elementwise_affine= False)
        ...
        if norm == "BatchNorm":
            ...
            self.norm2 = nn.LayerNorm([batch_size,F1*D,1,1025], eps = 1e-05, elementwise_affine= False)
        ...
        if norm == "BatchNorm":
            ...
            self.norm3 = nn.LayerNorm([batch_size,F2,1,241], eps = 1e-05, elementwise_affine= False)
    ...
    def EEGNet_update_batch_size(self, n_output,sample_rate,F1,D,F2,n_channels,bias,dropout,activation,norm,batch_size):
        print("Batch_size updated :",batch_size)
        # Block 1
        self.conv1LN = nn.Conv2d(in_channels = 1, out_channels = F1, kernel_size=(1,sample_rate//2), padding= 0,bias=False)
        self.norm1LN = nn.LayerNorm([batch_size,F1,n_channels,1025], eps = 1e-05, elementwise_affine= False)

        # DepthwiseConv2D
        self.conv2LN = nn.Conv2d(in_channels = F1, out_channels = F1*D, kernel_size=(n_channels,1), padding = 0,bias=False)
        self.norm2LN = nn.LayerNorm([batch_size,F1*D,1,1025], eps = 1e-05, elementwise_affine= False)
        
        #Activation function
        if activation == "SILU":
            self.activationLN = nn.SiLU()
        elif activation == "ELU":
            self.activationLN = nn.ELU()
        elif activation == "GELU":
            self.activationLN = nn.GELU()
        
        self.pool1LN = nn.AvgPool2d(kernel_size= (1,4))
        self.dropout1LN = nn.Dropout(p = dropout)

        # Block 2
        # SeparableConv2D
        self.depthwiseLN = nn.Conv2d(in_channels = F1*D, out_channels = F2, kernel_size=(1,sample_rate//8), padding=(0,16//2), groups = F2, bias=False) # Captures 500 ms of data at sampling rate 64Hz
        self.pointwiseLN = nn.Conv2d(in_channels = F1*D , out_channels = F2, kernel_size = 1)
        self.norm3LN = nn.LayerNorm([batch_size,F2,1,241], eps = 1e-05, elementwise_affine= False)
        
        if activation == "SILU":
            self.activation2LN = nn.SiLU()
        elif activation == "ELU":
            self.activation2LN = nn.ELU()
        elif activation == "GELU":
            self.activation2LN = nn.GELU()
        
        self.pool2LN = nn.AvgPool2d(kernel_size=(1,8))
        self.dropout2LN = nn.Dropout(p= dropout)
        self.flattenLN = nn.Flatten()

        # Classifier
        self.classifierLN = nn.Linear(1920,n_output,bias=bias)
        self.softmaxLN = nn.LogSoftmax(dim = 1)

    def forward_LN(self,x):
        # Block 1
        out = self.conv1LN(x)
        out = self.norm1LN(out)
        # DepthwiseConv2D
        out = self.conv2LN(out)
        out = torch.renorm(out , p=2, dim=0, maxnorm=1)
        out = self.norm2LN(out)
        out = self.activationLN(out)
        out = self.pool1LN(out)
        out = self.dropout1LN(out)
        # Block 2
        out = self.depthwiseLN(out)
        out = self.pointwiseLN(out)
        out = self.norm3LN(out)
        out = self.activation2LN(out)
        out = self.pool2LN(out)
        out = self.dropout2LN(out)
        out = self.flattenLN(out)
        # Classifier
        out = self.classifierLN(out)
        out = self.softmaxLN(out)
        out = torch.renorm(out, p=2,dim =0, maxnorm = 0.25)
...
def testing(x_test,y_test,model,device,criterion,n_gpu,n_output,sample_rate,F1,D,F2,n_channels,bias,dropout,activation,norm):
    with torch.no_grad():
        model.cuda(n_gpu)
        n = x_test.shape[0]
        ...
        if norm == "LayerNorm":
            model.EEGNet_update_batch_size(n_output,sample_rate,F1,D,F2,n_channels,bias,dropout,activation,norm,batch_size = x_test.shape[0])
            y_pred_test = model.forward_LN(x_test)
        else:
...
save_dir = str(activation)+'_dropout='+str(dropout)+'_F1='+str(F1)+'_D='+str(D)+'_lr='+str(lr)+'_bs='+str(batch_size)+'_bias='+str(bias)+'_'+str(chans)+'chans_NLLLoss'
...
for N_S in N_subj_arr:
    ...
    data_selec, labels = shuffle_in_unison(data_selec,labels)

    data_fold_arr = np.split(data_selec,n_splits,axis = 0)
    ...
    for offset in range(1,n_splits+1):
        ...
        model = EEGNet(n_output=n_class,sample_rate = sample_rate,F1 =F1,D=D,F2=F2,n_channels = chans, bias = bias, dropout = dropout, activation = activation, norm = norm,batch_size= batch_size)
        print(model)
        ...
        for epoch in range(epochs):
            ...
            print("labels_train length :",labels_train.shape)
            ...
            # Testing evaluation
            test_accuracy,test_loss = testing(X_test,Y_test,model,device,criterion,n_gpu,n_class,sample_rate,F1,D,F2,chans,bias,dropout,activation,norm)
